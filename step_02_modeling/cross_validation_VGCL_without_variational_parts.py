...
import math
from model_v3_GCN import Encoder_sc, Encoder_fc, train, test
...
for i in range(num_folds):
    ...
    for epoch in range(num_epochs):
        ...
        total_loss, total_contrastive_loss, total_d_loss_sc, total_d_loss_fc = 0, 0, 0, 0
        d_test = torch.tensor([])
        subjs_test = []
        for batch_sc, batch_fc in tqdm(zip(sc_test_dataloader, fc_test_dataloader), total=num_batches):
            ...
            loss, contrastive_loss, d_loss_sc, d_loss_fc, d_batch = test(model_sc, model_fc, batch_sc, batch_fc, coefs)
        ...
        print_test_loss = f'Loss: {total_loss/num_batches:.4f}, Contrastive Loss: {total_contrastive_loss/num_batches:.4f}, D Loss SC: {total_d_loss_sc/num_batches:.4f}, D Loss FC: {total_d_loss_fc/num_batches:.4f}'
