...
def train(model_sc, model_fc, optimizer_sc, optimizer_fc, batch_sc, batch_fc, coefs):
    ...
    contrastive_loss, d_loss_sc, kl_loss_sc, d_loss_fc, kl_loss_fc, _ = Loss(model_sc, model_fc, z_sc_nodes, z_fc_nodes, batch_sc, batch_fc)
...
def test(model_sc, model_fc, batch_sc, batch_fc, coefs):
    ...
    with torch.no_grad():
        ...
        contrastive_loss, d_loss_sc, kl_loss_sc, d_loss_fc, kl_loss_fc, d_nodes = Loss(model_sc, model_fc, z_sc_nodes, z_fc_nodes, batch_sc, batch_fc)
        loss = coefs[0]*contrastive_loss + coefs[1]*d_loss_sc + coefs[2]*kl_loss_sc + coefs[3]*d_loss_fc + coefs[4]*kl_loss_fc

        d_batch = d_nodes.reshape((-1, 360)).to('cpu')

        return float(loss.detach()), float(contrastive_loss.detach()), float(d_loss_sc.detach()), float(kl_loss_sc.detach()), float(d_loss_fc.detach()), float(kl_loss_fc.detach()), d_batch
...
def Loss(model_sc, model_fc, z_sc_nodes, z_fc_nodes, batch_sc, batch_fc):

    d_nodes = F.cosine_similarity(z_sc_nodes, z_fc_nodes, dim=-1) # torch.sum((z_sc_nodes-z_fc_nodes)**2, dim=1)
    contrastive_loss = 1 - d_nodes.mean()
...
def Distance_Loss_sc(z_sc_nodes, batch_sc):
    ...
    d_loss_sc = torch.mean((compute_distance(z_sc_nodes, batch_sc) - attr).abs(), dim=0)
...
def Distance_Loss_fc(z_fc_nodes, batch_fc):
    ...
    d_loss_fc = torch.mean((compute_distance(z_fc_nodes, batch_fc) - attr.abs()).abs(), dim=0)
...
def compute_distance(batch_z, batch):
    ...
    for graph_idx in range(batch_size):
        ...
        graph_z_norm = graph_z / graph_z.norm(dim=1, keepdim=True)
        distances[graph_idx] = torch.mm(graph_z_norm, graph_z_norm.t()) # torch.cdist(graph_z, graph_z)
