...
import math
from model_v3_VGAE import Encoder_sc, Encoder_fc, train, test
...
log = []
dm_sc_test_all = torch.tensor([])
dm_fc_test_all = torch.tensor([])
d_whole_test_all = torch.tensor([])
...
for i in range(num_folds):
    ...
    for epoch in range(num_epochs):
        ...
        d_test = torch.tensor([])
        subjs_test = []
        dm_sc_test = torch.tensor([])
        dm_fc_test = torch.tensor([])
        d_whole_test = torch.tensor([])
        for batch_sc, batch_fc in tqdm(zip(sc_test_dataloader, fc_test_dataloader), total=num_batches):

            assert [sc_id[:6] for sc_id in batch_sc.sample_id] == [fc_id[:6] for fc_id in batch_fc.sample_id]

            # loss, contrastive_loss, d_loss_sc, kl_loss_sc, d_loss_fc, kl_loss_fc, d_batch, dm_sc, dm_fc = test(model_sc, model_fc, batch_sc, batch_fc, coefs, dm=True)
            # loss, contrastive_loss, d_loss_sc, kl_loss_sc, d_loss_fc, kl_loss_fc, d_batch = test(model_sc, model_fc, batch_sc, batch_fc, coefs)
            loss, contrastive_loss, d_loss_sc, kl_loss_sc, d_loss_fc, kl_loss_fc, d_batch, d_whole_batch = test(model_sc, model_fc, batch_sc, batch_fc, coefs)    
            ...
            d_whole_test = torch.concat([d_whole_test, d_whole_batch], dim=0)

            # dm_sc_test = torch.concat([dm_sc_test, dm_sc], dim=0)
            # dm_fc_test = torch.concat([dm_fc_test, dm_fc], dim=0)
    ...
    d_test_all = torch.concat([d_test_all, d_test], dim=0)
    # dm_sc_test_all = torch.concat([dm_sc_test_all, dm_sc_test.mean(dim=0, keepdim=True)], dim=0)
    # dm_fc_test_all = torch.concat([dm_fc_test_all, dm_fc_test.mean(dim=0, keepdim=True)], dim=0)

    d_whole_test_all = torch.concat([d_whole_test_all, d_whole_test.mean(dim=0, keepdim=True)], dim=0)

    # np.save(f'D:\\Download\\d_whole_test_fold{i+1}.npy', d_whole_test_all.detach().numpy())
    # np.save(f'D:\\Download\\d_test_fold{i+1}.npy', d_test_all.detach().numpy())
...
np.save('D:\\Download\\distances.npy', d_test_all.detach().numpy())

# np.save('D:\\Download\\dm_sc.npy', dm_sc_test_all.detach().numpy())
# np.save('D:\\Download\\dm_fc.npy', dm_fc_test_all.detach().numpy())

# np.save('D:\\Download\\distances_whole.npy', d_whole_test_all.detach().numpy())
